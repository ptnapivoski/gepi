...
if($_SESSION['user']){
	...
	// Se conectado ao DB
	if($db_link){
		// País do qual selecionar unidades federativas
		$pais = (int) $_GET['pais'];

		// Tenta selecionar as unidades federativas daquele país
		if($db_query = mysqli_query($db_link, "SELECT id, nome, sigla FROM uf WHERE pais = $pais ORDER BY nome;")){
			...
			if(mysqli_num_rows($db_query)){
				...
				echo '<option value="0">Selecione uma unidade federativa</option>', $EOL;
				...
				while($db_result = mysqli_fetch_row($db_query)){
					...
					// Caso a unidade federativa tenha sigla
					if($db_result[2] !== '') $db_result[2] = " - $db_result[2]";

					// Exibe os valores
					echo '<option value="', $db_result[0], '">', $db_result[1], $db_result[2], '</option>', $EOL;
				}
