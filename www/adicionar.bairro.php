...
if($_SESSION['user']){
	...
	// Se conectado ao DB
	if($db_link){
		// Cidade na qual adicionar o bairro
		$id = (int) $_POST['cidade'];
		...
		if(perm($db_link, 'permissao_e_cidade', 46, $id)){
			// Valida dado vindo do formulário
			$nome = mysqli_real_escape_string($db_link, $_POST['nome']);
			if($nome === '') $nome = 'NULL'; else $nome = "'$nome'";

			// Tenta inserir
			if($db_query = mysqli_query($db_link, "INSERT INTO bairro (cidade, nome) VALUES ($id, $nome);")){
				...
				if(mysqli_affected_rows($db_link) === 1){
					...
					mysqli_query($db_link, "INSERT INTO permissao_e_bairro VALUES ($_SESSION[user], TRUE, 47, $n),($_SESSION[user], TRUE, 48, $n);");
