...
if($_SESSION['user']){
	...
	if($db_link){
		...
		if(perm($db_link, 'permissao_e_entidade', 78, $id)){
			...
			// Tenta checar se serviço é de defesa dos direitos da pessoa com deficiência
			if($db_query = mysqli_query($db_link, "SELECT NULL FROM servico WHERE id = $servico AND tipo_de_servico = 2;")){
				// Se selecionou serviço de defesa dos direitos da pessoa com deficiência
				...
				} else $_SESSION['msg'] = '<p class="error">Serviço não é de defesa dos direitos da pessoa com deficiência.</p>';
			// Caso não tenha conseguido realizar a consulta
			} else {
				// Seleciona-se e escapa-se o erro
				$error = htmlspecialchars(mysqli_error($db_link));
