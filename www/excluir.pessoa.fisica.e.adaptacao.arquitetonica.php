...
if($_SESSION['user']){
	...
	if($db_link){
		...
		if(perm($db_link, 'permissao_e_entidade', 91, $id)){
			...
			// Tenta excluir
			if($db_query = mysqli_query($db_link, "DELETE FROM pessoa_fisica_e_adaptacao_arquitetonica WHERE pessoa_fisica = $id AND adaptacao_arquitetonica = $adaptacao_arquitetonica;")){
