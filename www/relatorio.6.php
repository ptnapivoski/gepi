...
if($_SESSION['user']){
	...
	if($db_link){
		...
		if($tecnologia > 0){
			...
			if($db_query = mysqli_query($db_link, "SELECT pfe.frequencia FROM pessoa_fisica_e_tecnologia pft INNER JOIN pessoa_fisica_e_escola pfe ON pfe.pessoa_fisica = pft.pessoa_fisica WHERE pft.tecnologia = $tecnologia AND pfe.ano = $ano ORDER BY pfe.frequencia ASC;")){
