...
if($_SESSION['user']){
	...
	if($db_link){
		...
		if(perm($db_link, 'permissao_e_diagnostico', 7, NULL)){
			// Valida dado vindo do formulário
			$nome = mysqli_real_escape_string($db_link, $_POST['nome']);
			if($nome === '') $nome = 'NULL'; else $nome = "'$nome'";
			$cid = mysqli_real_escape_string($db_link, $_POST['cid']);
			if($cid === '') $cid = 'NULL'; else $cid = "'$cid'";

			// Tenta inserir
			if($db_query = mysqli_query($db_link, "INSERT INTO diagnostico (nome, cid) VALUES ($nome, $cid);")){
				...
				if(mysqli_affected_rows($db_link) === 1){
					...
					mysqli_query($db_link, "INSERT INTO permissao_e_diagnostico VALUES ($_SESSION[user], TRUE, 8, $n),($_SESSION[user], TRUE, 9, $n);");
