...
if($_SESSION['user']){
	...
	// Se conectado ao DB
	if($db_link){
		// Pessoa a qual selecionar
		$id = (int) $_GET['id'];

		// Tenta selecionar a pessoa
		if($db_query = mysqli_query($db_link, "SELECT nome FROM entidade WHERE id = $id AND tipo_de_entidade = 1;")){
			...
			if(mysqli_num_rows($db_query)){
				...
				// E os exibe
				echo $db_result[0];
			// Ao não encontrar a pessoa
			} else echo 'Pessoa inválida';
