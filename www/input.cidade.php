...
if($_SESSION['user']){
	...
	// Se conectado ao DB
	if($db_link){
		// Cidade a qual selecionar
		$id = (int) $_GET['id'];

		// Tenta selecionar a cidade, seu estado e seu país
		if($db_query = mysqli_query($db_link, "SELECT pais.nome, uf.nome, cid.nome FROM cidade cid LEFT JOIN uf ON uf.id = cid.uf LEFT JOIN pais ON pais.id = uf.pais WHERE cid.id = $id;")){
			...
			if(mysqli_num_rows($db_query)){
				...
				// E os exibe
				echo $nome;
			// Ao não encontrar a cidade
			} else echo 'Cidade inválida';
