...
// Se logado
if($_SESSION['user']){
	// Pessoa à qual adicionar necessidade de adaptação arquitetônica
	$id = (int) $_POST['id'];
	...
	if($db_link){
		...
		if(perm($db_link, 'permissao_e_entidade', 91, $id)){
			...
			// Tenta inserir
			if($db_query = mysqli_query($db_link, "INSERT INTO pessoa_fisica_e_adaptacao_arquitetonica (pessoa_fisica, adaptacao_arquitetonica) VALUES ($id, $adaptacao_arquitetonica);")){
