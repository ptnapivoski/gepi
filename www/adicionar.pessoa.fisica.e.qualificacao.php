...
// Se logado
if($_SESSION['user']){
	// Pessoa à qual adicionar qualificação
	$id = (int) $_POST['id'];
	...
	if($db_link){
		...
		if(perm($db_link, 'permissao_e_entidade', 80, $id)){
			...
			$qualificacao = (int) $_POST['qualificacao'];

			// Tenta inserir
			if($db_query = mysqli_query($db_link, "INSERT INTO pessoa_fisica_e_qualificacao (pessoa_fisica, qualificacao) VALUES ($id, $qualificacao);")){
