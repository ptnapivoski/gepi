...
if($_SESSION['user']){
	...
	// Se conectado ao DB
	if($db_link){
		// Logradouro a selecionar
		$id = (int) $_GET['id'];

		// Tenta selecionar o logradouro
		if($db_query = mysqli_query($db_link, "SELECT nome FROM logradouro WHERE id = $id;")){
			...
			if(mysqli_num_rows($db_query)){
				...
				// E os exibe
				echo $db_result[0];
			// Ao não encontrar o logradouro
			} else echo 'Logradouro inválido';
