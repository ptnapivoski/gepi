...
if($_SESSION['user']){
	...
	if($db_link){
		...
		if(perm($db_link, 'permissao_e_status_de_diagnostico', 10, NULL)){
			// Valida dado vindo do formulário
			$nome = mysqli_real_escape_string($db_link, $_POST['nome']);
			if($nome === '') $nome = 'NULL'; else $nome = "'$nome'";

			// Tenta inserir
			if($db_query = mysqli_query($db_link, "INSERT INTO status_de_diagnostico (nome) VALUES ($nome);")){
				...
				if(mysqli_affected_rows($db_link) === 1){
					...
					mysqli_query($db_link, "INSERT INTO permissao_e_status_de_diagnostico VALUES ($_SESSION[user], TRUE, 11, $n),($_SESSION[user], TRUE, 12, $n);");
