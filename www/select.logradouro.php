...
if($_SESSION['user']){
	...
	// Se conectado ao DB
	if($db_link){
		// Cidade da qual selecionar logradouros
		$cidade = (int) $_GET['cidade'];
		// Tipo de logradouro a selecionar
		$tipo_de_logradouro = (int) $_GET['tipo_de_logradouro'];
		// Filtro pelo nome do logradouro
		$filtro = mysqli_real_escape_string($db_link, $_GET['filtro']);

		// Tenta selecionar os logradouros daquela cidade e daquele tipo
		if($db_query = mysqli_query($db_link, "SELECT id, nome FROM logradouro WHERE cidade = $cidade AND tipo_de_logradouro = $tipo_de_logradouro AND nome LIKE '%$filtro%' ORDER BY nome;")){
			// Se selecionou pelo menos um
			if(mysqli_num_rows($db_query)){
				// Primeira opção vazia
				echo '<option value="0">Selecione um logradouro</option>', $EOL;
