...
if($_SESSION['user']){
	...
	if($db_link){
		...
		if(perm($db_link, 'permissao_e_entidade', 76, $id)){
			...
			// Tenta checar se serviço é de educação
			if($db_query = mysqli_query($db_link, "SELECT NULL FROM servico WHERE id = $servico AND tipo_de_servico = 3;")){
				// Se selecionou serviço de educação
				...
				} else $_SESSION['msg'] = '<p class="error">Serviço não é de educação.</p>';
			// Caso não tenha conseguido realizar a consulta
			} else {
				// Seleciona-se e escapa-se o erro
				$error = htmlspecialchars(mysqli_error($db_link));
