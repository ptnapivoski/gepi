...
if($_SESSION['user']){
	...
	// Se conectado ao DB
	if($db_link){
		// Escola a qual selecionar
		$id = (int) $_GET['id'];

		// Tenta selecionar a escola
		if($db_query = mysqli_query($db_link, "SELECT nome FROM entidade WHERE id = $id AND tipo_de_entidade IN (5,6,7,8,9);")){
			...
			if(mysqli_num_rows($db_query)){
				...
				// E os exibe
				echo $db_result[0];
			// Ao não encontrar a escola
			} else echo 'Escola inválida';
