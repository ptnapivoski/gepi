...
if($_SESSION['user']){
	...
	// Se conectado ao DB
	if($db_link){
		// País no qual adicionar a unidade federativa
		$id = (int) $_POST['pais'];
		...
		if(perm($db_link, 'permissao_e_pais', 34, $id)){
			// Valida dado vindo do formulário
			$nome = mysqli_real_escape_string($db_link, $_POST['nome']);
			if($nome === '') $nome = 'NULL'; else $nome = "'$nome'";
			$sigla = mysqli_real_escape_string($db_link, $_POST['sigla']);
			if($sigla === '') $sigla = 'NULL'; else $sigla = "'$sigla'";

			// Tenta inserir
			if($db_query = mysqli_query($db_link, "INSERT INTO uf (pais, nome, sigla) VALUES ($id, $nome, $sigla);")){
				...
				if(mysqli_affected_rows($db_link) === 1){
					...
					mysqli_query($db_link, "INSERT INTO permissao_e_uf VALUES ($_SESSION[user], TRUE, 35, $n),($_SESSION[user], TRUE, 36, $n);");
