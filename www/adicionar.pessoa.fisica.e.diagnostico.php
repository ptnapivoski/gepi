...
if($_SESSION['user']){
	...
	if($db_link){
		...
		if(perm($db_link, 'permissao_e_entidade', 68, $id)){
			...
			if($permanente === '1') $permanente = 'TRUE';
			else $permanente = 'FALSE';

			// Tenta inserir
			if($db_query = mysqli_query($db_link, "INSERT INTO pessoa_fisica_e_diagnostico (pessoa_fisica, diagnostico, permanente, status_de_diagnostico) VALUES ($id, $diagnostico, $permanente, $status_de_diagnostico);")){
