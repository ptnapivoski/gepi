...
if($_SESSION['user']){
	...
	// Se conectado ao DB
	if($db_link){
		// Unidade federativa a qual selecionar
		$id = (int) $_GET['id'];

		// Tenta selecionar a unidade federativa
		if($db_query = mysqli_query($db_link, "SELECT nome FROM uf WHERE id = $id;")){
			...
			if(mysqli_num_rows($db_query)){
				...
				// E os exibe
				echo $db_result[0];
			// Ao não encontrar a unidade federativa
			} else echo 'Unidade federativa inválida';
