...
if($_SESSION['user']){
	...
	if($db_link){
		...
		if(perm($db_link, 'permissao_e_entidade', 87, NULL)){
			...
			// Se ação válida
			if(in_array($acao, array(110,111,112), TRUE)){
				...
				if($db_query = mysqli_query($db_link, "SELECT tem_objeto FROM acao WHERE id = $acao;")){
					...
					if(mysqli_num_rows($db_query) === 1){
						...
						// Tenta inserir
						if($db_query_inc = mysqli_query($db_link, "INSERT INTO permissao_e_adaptacao_arquitetonica VALUES ($entidade,$pode,$acao,$obj);")){
