...
if($_SESSION['user']){
	...
	if($db_link){
		...
		if(perm($db_link, 'permissao_e_entidade', 56, $id)){
			...
			$telefone = mysqli_real_escape_string($db_link, $_POST['telefone']);
			if($telefone === '') $telefone = 'NULL'; else $telefone = "'$telefone'";
			$obs = mysqli_real_escape_string($db_link, $_POST['obs']);
			if($obs === '') $obs = 'NULL'; else $obs = "'$obs'";

			// Tenta inserir
			if($db_query = mysqli_query($db_link, "INSERT INTO telefone (entidade, telefone, obs) VALUES ($id, $telefone, $obs);")){
