...
if($_SESSION['user']){
	...
	if($db_link){
		...
		if(perm($db_link, 'permissao_e_turno_escolar', 19, NULL)){
			// Valida dado vindo do formulário
			$nome = mysqli_real_escape_string($db_link, $_POST['nome']);
			if($nome === '') $nome = 'NULL'; else $nome = "'$nome'";

			// Tenta inserir
			if($db_query = mysqli_query($db_link, "INSERT INTO turno_escolar (nome) VALUES ($nome);")){
				...
				if(mysqli_affected_rows($db_link) === 1){
					...
					mysqli_query($db_link, "INSERT INTO permissao_e_turno_escolar VALUES ($_SESSION[user], TRUE, 20, $n),($_SESSION[user], TRUE, 21, $n);");
