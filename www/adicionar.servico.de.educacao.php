...
if($_SESSION['user']){
	...
	if($db_link){
		...
		if(perm($db_link, 'permissao_e_servico_de_educacao', 97, NULL)){
			// Valida dado vindo do formulário
			$nome = mysqli_real_escape_string($db_link, $_POST['nome']);
			if($nome === '') $nome = 'NULL'; else $nome = "'$nome'";

			// Tenta inserir
			if($db_query = mysqli_query($db_link, "INSERT INTO servico_de_educacao (nome) VALUES ($nome);")){
				...
				if(mysqli_affected_rows($db_link) === 1){
					...
					mysqli_query($db_link, "INSERT INTO permissao_e_servico_de_educacao VALUES ($_SESSION[user], TRUE, 98, $n),($_SESSION[user], TRUE, 99, $n);");
