...
// Se logado
if($_SESSION['user']){
	// Pessoa à qual adicionar interesse em qualificação
	$id = (int) $_POST['id'];
	...
	if($db_link){
		...
		if(perm($db_link, 'permissao_e_entidade', 74, $id)){
			...
			// Tenta inserir
			if($db_query = mysqli_query($db_link, "INSERT INTO pessoa_fisica_e_interesse_em_qualificacao (pessoa_fisica, interesse) VALUES ($id, $interesse);")){
