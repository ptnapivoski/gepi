...
if($_SESSION['user']){
	...
	if($db_link){
		...
		echo
				...
				'</form>', $EOL,
			'</section>', $EOL,
			// Inicia a seção dos serviços
			'<section class="cad" id="servico-sec">', $EOL,
				'<h1>Serviços</h1>', $EOL
		;

		// Tenta selecionar os serviços
		if($db_query = mysqli_query($db_link, "SELECT ser.id, tds.nome, ser.nome FROM servico ser LEFT JOIN tipo_de_servico tds ON tds.id = ser.tipo_de_servico ORDER BY tds.nome, ser.nome;")){
			// Se selecionou pelo menos uma linha
			if(mysqli_num_rows($db_query)){
				// Inicia a tabela
				echo '<table>', $EOL;
				...
				while($db_result = mysqli_fetch_row($db_query)){
					...
					echo
						'<tr>', $EOL,
							'<td>', $EOL,
								'<form action="alterar.servico.php" method="post">', $EOL,
									'<input type="hidden" name="id" value="', $db_result[0], '"/>',
									'<input type="text" readonly="readonly" value="', $db_result[1], '"/> ',
									'<input type="text" name="nome" id="servico-', $db_result[0], '" required="required" value="', $db_result[2], '"/> ',
									'<input type="submit" value="Alterar" onclick="if($(\'#servico-', $db_result[0], '\').val()) return confirm(\'Tem certeza que deseja alterar o serviço \\\'', str_replace('\'', '\\\'', $db_result[2]), '\\\'?\');"/>', $EOL,
								'</form>', $EOL,
							'</td>', $EOL,
							'<td>', $EOL,
								'<form action="excluir.servico.php" method="post">', $EOL,
									'<input type="hidden" name="id" value="', $db_result[0], '"/>',
									'<input type="submit" value="Excluir" onclick="return confirm(\'Tem certeza que deseja excluir o serviço \\\'', str_replace('\'', '\\\'', $db_result[2]), '\\\'?\');"/>', $EOL,
								'</form>', $EOL,
							'</td>', $EOL,
						'</tr>', $EOL
					;
		...
		if($db_query = mysqli_query($db_link, "SELECT id, nome FROM tipo_de_servico ORDER BY nome;")){
			...
			if(mysqli_num_rows($db_query)){
				...
				echo
					'<form action="adicionar.servico.php" method="post" class="new">', $EOL,
						'<p>',
							'<label>Novo: ',
							'<select name="tipo_de_servico">',
				...
				while($db_result = mysqli_fetch_row($db_query)){
					...
					$db_result[1] = htmlspecialchars($db_result[1]);
					// Imprime opção
					echo '<option value="', $db_result[0], '">', $db_result[1], '</option>';
				}

				echo
							'</select> ',
							'<input type="text" name="nome" id="novo-servico" required="required" value="" /></label> ',
							'<input type="submit" value="Inserir" onclick="if($(\'#novo-servico\').val()) return confirm(\'Tem certeza que deseja inserir o serviço \\\'\' + $(\'#novo-servico\').val() + \'\\\'?\');"/>',
						'</p>', $EOL,
					'</form>', $EOL
				;
			// Se não selecionou nenhuma linha
