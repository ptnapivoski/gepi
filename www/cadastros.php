...
if($_SESSION['user']){
	...
	if($db_link){
						...
						'<tr>', $EOL,
							'<td>', $EOL,
								'<form action="alterar.diagnostico.php" method="post">', $EOL,
									...
									'<input type="text" name="nome" id="diagnostico-', $db_result[0], '" required="required" value="', $db_result[1], '" style="width:600px"/> ',
						...
						'<tr>', $EOL,
							'<td>', $EOL,
								'<form action="alterar.servico.de.saude.php" method="post">', $EOL,
									'<input type="hidden" name="id" value="', $db_result[0], '"/>',
									'<input type="text" name="nome" id="servico-de-saude-', $db_result[0], '" required="required" value="', $db_result[1], '" style="width:500px"/> ',
						...
						'<tr>', $EOL,
							'<td>', $EOL,
								'<form action="alterar.servico.de.as.php" method="post">', $EOL,
									'<input type="hidden" name="id" value="', $db_result[0], '"/>',
									'<input type="text" name="nome" id="servico-de-as-', $db_result[0], '" required="required" value="', $db_result[1], '" style="width:400px"/> ',
		...
		echo
				...
				'</form>', $EOL,
			'</section>', $EOL,
			// Inicia a seção dos serviços de habitação
			'<section class="cad" id="servico-de-hab-sec">', $EOL,
				'<h1>Serviços de Habitação</h1>', $EOL
		;

		// Tenta selecionar os serviços de habitação
		if($db_query = mysqli_query($db_link, "SELECT id, nome FROM servico_de_hab ORDER BY nome;")){
			// Se selecionou pelo menos uma linha
			if(mysqli_num_rows($db_query)){
				// Inicia a tabela
				echo '<table>', $EOL;
				...
				while($db_result = mysqli_fetch_row($db_query)){
					...
					echo
						'<tr>', $EOL,
							'<td>', $EOL,
								'<form action="alterar.servico.de.hab.php" method="post">', $EOL,
									'<input type="hidden" name="id" value="', $db_result[0], '"/>',
									'<input type="text" name="nome" id="servico-de-hab-', $db_result[0], '" required="required" value="', $db_result[1], '"/> ',
									'<input type="submit" value="Alterar" onclick="if($(\'#servico-de-hab-', $db_result[0], '\').val()) return confirm(\'Tem certeza que deseja alterar o serviço de habitação \\\'', str_replace('\'', '\\\'', $db_result[1]), '\\\'?\');"/>', $EOL,
								'</form>', $EOL,
							'</td>', $EOL,
							'<td>', $EOL,
								'<form action="excluir.servico.de.hab.php" method="post">', $EOL,
									'<input type="hidden" name="id" value="', $db_result[0], '"/>',
									'<input type="submit" value="Excluir" onclick="return confirm(\'Tem certeza que deseja excluir o serviço de habitação \\\'', str_replace('\'', '\\\'', $db_result[1]), '\\\'?\');"/>', $EOL,
								'</form>', $EOL,
							'</td>', $EOL,
						'</tr>', $EOL
					;
		...
		// Formulário para inserção e finaliza seção
		echo
				'<form action="adicionar.servico.de.hab.php" class="new" method="post">', $EOL,
					'<p>',
						'<label>Novo: <input type="text" name="nome" id="novo-servico-de-hab" required="required" value="" /></label> ',
						'<input type="submit" value="Inserir" onclick="if($(\'#novo-servico-de-hab\').val()) return confirm(\'Tem certeza que deseja inserir o serviço de habitação \\\'\' + $(\'#novo-servico-de-hab\').val() + \'\\\'?\');"/>',
					'</p>', $EOL,
				'</form>', $EOL,
			'</section>', $EOL
		;
